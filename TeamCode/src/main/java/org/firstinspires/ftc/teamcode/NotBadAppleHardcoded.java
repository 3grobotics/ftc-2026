...
@TeleOp(name = "not Bad Apple Hardcoded", group = "Meme")
public class NotBadAppleHardcoded extends OpMode {
    ...
    public void init() {
        ...
        telemetry.addLine("Loaded " + NotBadAppleData.FRAMES.length + " frames into memory.");
        telemetry.update();
    }

    @Override
    public void loop() {
        if (currentFrame >= NotBadAppleData.FRAMES.length) {
        ...
        if (frameTimer.milliseconds() >= FRAME_TIME_MS) {
            ...
            byte[] frameData = Base64.decode(NotBadAppleData.FRAMES[currentFrame], Base64.DEFAULT);
            int bitIndex = 0;
