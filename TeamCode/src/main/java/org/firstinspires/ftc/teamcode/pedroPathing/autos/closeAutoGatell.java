...
public class closeAutoGatell extends LinearOpMode {
    ...
    // PredominantColorProcessor results
    ...
    // --- NON-BLOCKING INTAKE STATE MACHINE ---
    enum IntakeState {
        IDLE,
        INTAKE_TILL_PPG,
        INTAKE_TILL_PGP,
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            // Trigger the state machine when all sensors hit
            if (all && currentIntakeState == IntakeState.IDLE) {
                if (motifSum == 44) {
            ...
            switch (currentIntakeState) {
                case IDLE:
                    // Let pathing/autonomous actions control hardware here
                    break;

                case INTAKE_TILL_PPG:
                case INTAKE_TILL_PGP:
                case INTAKE_TILL_GPP:
                    boolean matchedColor = false;

                    // Instantly check for color match every loop
                    if (currentIntakeState == IntakeState.INTAKE_TILL_PPG) {
                        if (frontResult != null && frontResult.closestSwatch.equals(PredominantColorProcessor.Swatch.ARTIFACT_GREEN)) matchedColor = true;
                    } else if (currentIntakeState == IntakeState.INTAKE_TILL_PGP) {
                        if (backResult != null && backResult.closestSwatch.equals(PredominantColorProcessor.Swatch.ARTIFACT_GREEN)) matchedColor = true;
                    } else if (currentIntakeState == IntakeState.INTAKE_TILL_GPP) {
                        if (frontResult != null && backResult != null &&
                                frontResult.closestSwatch.equals(PredominantColorProcessor.Swatch.ARTIFACT_PURPLE) &&
                                backResult.closestSwatch.equals(PredominantColorProcessor.Swatch.ARTIFACT_PURPLE)) matchedColor = true;
                    }
                    ...
                    if (matchedColor) {
                        ...
                        break; // Exit state execution, color acquired!
                    }

                    // Cycle Sub-Steps from TeleOp
                    switch (cycleSubStep) {
                        case 0:
                            h.swingArm.setPosition(1);
                        ...
                        case 8:
                            if (intakeStateTimer.milliseconds() > 100) {
                                h.intake.setPower(0);
                                cycleSubStep = 0; // Loop back to check color and reset arm
                            }
                            break;
                    }
                    break;
            ...
            panelsTelemetry.debug("Intake State", currentIntakeState.name());
            panelsTelemetry.update(telemetry);
        }
    ...
    public static class Paths {
        ...
        public Paths(Follower follower) {
            Path1 = follower.pathBuilder()
                    ...
                    .addPoseCallback(new Pose(86.000, 86.000, Math.toRadians(36)), () -> follower.pausePathFollowing(), .5)
