...
public class ColorCamera extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive() || opModeInInit()) {
            ...
            if (cycleState == 0) { // Only allow manual control if the cycle is NOT running
                ...
                if (gamepad1.right_bumper) {
                    ...
                    if (swingTimer.milliseconds() >= 100) {
                        swingHigh = !swingHigh;
                        swingTimer.reset();
                    }

                    h.swingArm.setPosition(swingHigh ? 0.9 : 0.8);
                ...
                if (gamepad1.ps) {
                    intakeTimer.resetTimer();
                    h.gate.setPosition(0.65);
                    while ((h.topDistSensor.getState() && h.midDistSensor.getState() && h.frontDistSensor.getState()) == false && intakeTimer.getElapsedTime() < 1000) {
                        h.intake.setPower(-1);
                        h.indexer.setPower(-1);
                    }
                    h.indexer.setPower(0);
            ...
            // cycles once trigger
            if (gamepad1.dpad_left && cycleState == 0) {
                cycleState = 1;
                cycleTimer.reset();
                h.sickle.setPosition(0.9);
                h.swingArm.setPosition(0.5);
            ...
            // cycles once (Switch/Case State Machine)
            switch (cycleState) {
                case 0:
                ...
                case 1:
                    if (cycleTimer.milliseconds() >= 250) {
                        h.intake.setPower(-1);
                        h.indexer.setPower(-1);
                        cycleState = 2;
                        cycleTimer.reset();
                ...
                case 4:
                    if (cycleTimer.milliseconds() >= 100) {
                        h.indexer.setPower(1);
                        cycleState = 5;
                        cycleTimer.reset();
                    }
            ...
            if (gamepad2.a || gamepad1.touchpad && !bPressed) {
                ptoState = (ptoState + 1) % 2;
                bPressed = true;
            } else if (!gamepad2.a || !gamepad1.touchpad) {
                bPressed = false;
            }
